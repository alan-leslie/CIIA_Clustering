...
public class AverageLinkAlgorithm {
    ...
    public AverageLinkAlgorithm(MultiDimensionalDataPoint[] elements, double[][] adjacencyMatrix) {
    ...
    private void mergeClusters(double distanceThreshold) {
        ...
        for(int i = 0, n = a.length; i < n; i++) {
            for(int j = i + 1, k = a.length; j < k; j++) {
                double d = a[i][j];
                if( d > 0 ) {
                    MultiDimensionalDataPoint e1 = elements[i];
                    MultiDimensionalDataPoint e2 = elements[j];
